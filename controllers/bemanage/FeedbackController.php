...
class FeedbackController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if(isset($image)) {
                ...
                $model->imgUrl = Yii::$app->params['uploadFeedbackPath'] . date('YmdHis', time()).mt_rand(1000,9999).".{$ext}";
                $path = '.' . $model->imgUrl;
            }
            if($model->save()){
    ...
    public function actionChangeStatus()
    {
        ...
        $rs = Feedback::updateAll(['status' => $status],['in','id',$ids]);
